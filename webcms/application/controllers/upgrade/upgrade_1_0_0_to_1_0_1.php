...
class Upgrade_1_0_0_to_1_0_1 extends CI_Controller
{
	...
	{
		$this->db->simple_query("CREATE TABLE `events` (
		  ...
		  `subname_lang` varchar(255) NOT NULL DEFAULT '',
		...
		$this->db->simple_query("ALTER TABLE `image` ADD COLUMN `events_id` INT(10) UNSIGNED DEFAULT 0 NOT NULL AFTER `author_id`, ADD COLUMN `events_type` VARCHAR(128) DEFAULT '' NOT NULL AFTER `author_name`, ADD COLUMN `events_number` VARCHAR(128) DEFAULT '' NOT NULL AFTER `events_type`, ADD COLUMN `events_name` VARCHAR(255) DEFAULT '' NOT NULL AFTER `events_number`, ADD COLUMN `events_date` INT(10) DEFAULT 0 NOT NULL AFTER `events_name`, ADD COLUMN `events_status` VARCHAR(128) DEFAULT '' NOT NULL AFTER `events_date`;");
	}
}