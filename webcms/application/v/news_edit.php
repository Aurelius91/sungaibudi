	...
	<script type="text/javascript">
		...
		function reset() {
			$('#news-name').val("<?= $news->name; ?>");
			$('#news-date').val("<?= $news->date_display; ?>");
			$('#news-author').val("<?= $news->author; ?>");

			$('#news-name-lang').val("<?= $news->name_lang; ?>");

			$('#news-metatag-title').val("<?= $news->metatag_title; ?>");
			$('#news-metatag-author').val("<?= $news->metatag_author; ?>");
			$('#news-metatag-keywords').val("<?= $news->metatag_keywords; ?>");
			$('#news-metatag-description').val("<?= $news->metatag_description; ?>");

			$('#main-image').val("");
			$('#preview').data('image_id', 0);
			$('.preview').remove();

			var image = '<img class="preview" src="<?= base_url(); ?>images/website/<?= $news->image_name; ?>">';
			$('#preview').append(image);
		}
		...
		function submit() {
			...
			$.ajax({
				...
				type : 'POST',
				url : '<?= base_url() ?>news/ajax_edit/<?= $news->id; ?>',
				xhr: function() {
					var percentage = 0;
					var xhr = new window.XMLHttpRequest();
					...
					<div class="ui form">
						...
						<div class="field">
							...
							<div class="field">
								...
								<textarea id="news-content" class="form-input" placeholder="Content.." type="text"><?= $news->description; ?></textarea>
							</div>
						</div>
						...
						<div class="field">
							...
							<div class="field">
								<label>Content</label>
								<textarea id="news-content-lang" class="form-input" placeholder="Content.." type="text"><?= $news->description_lang; ?></textarea>
							</div>
						</div>
