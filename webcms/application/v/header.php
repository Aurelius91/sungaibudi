	...
	<script type="text/javascript">
		...
		function addHeader() {
			...
			var headerNameLang = $('#header-name-lang-add').val();
		...
		function editHeader() {
			...
			var headerNameLang = $('#header-name-lang-edit').val();
			/*var headerImage = $('#header-image-edit').data('image_id');*/
			...
			$.ajax({
				...
				success: function(data){
					if (data.status == 'success') {
						...
						$('#header-name-lang-edit').val(data.header.name_lang);
						$('#header-name-edit').data('header_id', headerId);
						$('#header-type-container-edit').dropdown('set selected', data.header.type);
						$('#header-image-edit').val("");
						$('#header-image-edit').data('image_id', 0);
					...
					<div class="ui form">
						<div class="two fields">
							...
							<div class="field">
								...
								<input id="header-name-lang-add" type="text" placeholder="Name..">
							</div>
						</div>
					</div>
				</div>
				...
				<div class="form-content">
					<div class="ui form">
						<div class="two fields">
							...
							<div class="field">
								<label>Name - <?= $setting->setting__system_language2; ?> <span class="color-red warning"></span></label>
								<input id="header-name-lang-edit" placeholder="Name..">
							</div>
						</div>
					</div>
				</div>
				...
				<tbody>
					...
					<? else: ?>
						<? foreach ($arr_header as $header): ?>
							...
							<? foreach ($header->arr_child_header as $child_header): ?>
								<tr>
									<td class="td-icon">
										<? if (isset($acl['website']) && $acl['website']->edit > 0): ?>
											<a onclick="getheader('<?= $child_header->id; ?>');">
												<span class="table-icon">
													<i class="edit icon"></i>
												</span>
											</a>
									...
									<td>--> <?= $child_header->name; ?></td>
									<td><?= $child_header->type; ?></td>
								</tr>
							<? endforeach; ?>
						<? endforeach; ?>
					<? endif; ?>
